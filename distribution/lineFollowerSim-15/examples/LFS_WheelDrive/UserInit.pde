...
void userInit()  // called at startup and typically by UserReset tab's userControllerResetAndRun method
{
  ...
  lfs.setFirstNameLastNameRobotName("Your"," Name","WheelLR");   // change to your name and robot
  ...
  lfs.chooseCourseOneTime (1);          // Here you choose an initial course to use. 
  ...
  sensorL = lfs.createSpotSensor(1,-2,20,20);         // example spot sensors 
  sensorM = lfs.createSpotSensor(1.5f,0,15,15);
  sensorR = lfs.createSpotSensor(1, 2,20,20);
  ...
  */    
  ...
  if (genIcon)
  {
    String genInitials = "L-R";               // replace with your initials (2 or 3 char) and colors below... 
    color genColor = color (250,40,0);      // robot color  Aztec Yellow
