/* Line Following Simulator User Application - Processing Environment
   ...
   LFS_ boilerplate ALMOST 
   Made change to LFS_Key to send cursor arrow keys to UKey method for decode 
   ...
   * out of bounds detection added
   ...
   uXXX     lowercase uName is user code. 
   
   For Trike :
   uSensor     Sensor low level data interpreter 
   uTracking   State Space Controller
   uTrike      Trike class
...
*/  
  /* Line Following Simulator User Application - Processing Environment
   ...
   SimpleBot5 - 1.4.2 and later  Lap Timer, Improved Run - Faster 
   ...
   * out of bounds detection added
   ...
   - UserDraw UserReset UserCon left alone, minor change to UserInit
...
void settings()
{
  if (maximizedWindow) fullScreen(P3D);
...
public void setup()
{
  ...
  frameRate(1000);           // request high frame rate to run simulator potentially faster 
                             // than realtime resulting frames per second can be very high, reported on screen (top right fps)  
  ...
  public void draw ()  // method called by Processing as fast as possible, unless throttled by setup call to frameRate()
                       // typically 30 to 100 or more times per second (called frame rate) 
  {
