...
void uprintln (String s)    // user panel println,  does println AND shows on screen 
{  userPanelString = s;
   println (s); // print to console  
}  
                               ...
                               // parameters not visible
{
  ...
  int ypos = 30;
  text ("UPanel  Trike Data ",20,ypos);
  ypos += 30;
  text (String.format("Trike Wheel Speed %1.2f  wheel angle %1.1f  ",
                       trike.wheelVelocity,degrees(trike.steerAngleR)),20,ypos);
  ypos+= 30;
  text (userPanelString,20,ypos);
