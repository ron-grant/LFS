/* LFS_Panel (formerly UserDrawPanel)

   Now UPanel is exclusively for user panel drawing.

   For Library 1.4  Reworked this tab
   Modified for ParEdit & New H)elp screen  library 1.4
   
   Command Summary Text and Help Text defined in this file.
   Coordinate transforms and calls out to user panel drawing provided in this module (tab).
...
void lfsDrawPanel()  // called from draw() at frame rate
{
  ...
  if (helpPage>0) // draw Help if page 1 or 2 controlld by keypress H, else command summary
  {
    ...
    else
    {
      ...
      cmdSum ("LFS Command Key Summary        H)elp       P)arameter dialog");
      cmdSumGap();
